...
template<int WIDTH, int HEIGHT, typename Precision = float>
__global__ void fused_fft_conv_kernel_C2C(
    const Precision* __restrict__ input
    , const Precision* __restrict__ kernel
    , Precision* __restrict__ output
    )
{
    using Complex = cufftdx::complex<Precision>;

    using RowFFT = decltype(FFT()
        .size<WIDTH>()
        .type<fft_type::c2c>()
    ...
    __shared__ typename RowFFT::shared_memory_type shared_row;
    __shared__ typename ColFFT::shared_memory_type shared_col;
    ...
    // step 1: real -> complex
    Complex val_in = { input[idx], 0.0 };   
    Complex val_ker = { kernel[idx], 0.0 };

    // step 2: row fft
    row_fft.execute(val_in, shared_row);
    row_fft.execute(val_ker, shared_row);
    __syncthreads();

    // step 3: column fft
    col_fft.execute(val_in, shared_col);
    col_fft.execute(val_ker, shared_col);

    // step 4: pointwise multiplication
    Complex mult;    
    mult.x = val_in.x * val_ker.x - val_in.y * val_ker.y;
    mult.y = val_in.x * val_ker.y + val_in.y * val_ker.x;

    // step 5: ifft column
    using IFFT_Col = decltype(FFT()
        .size<HEIGHT>()
        .type<fft_type::c2c>()
    ...
    IFFT_Col ifft_col;
    ifft_col.execute(mult, shared_col);
    
    // step 6: ifft row
    using IFFT_Row = decltype(FFT()
        .size<WIDTH>()  
        .type<fft_type::c2c>()
        .direction<fft_direction::inverse>()
        .precision<Precision>()
    ...
    IFFT_Row ifft_row;
    ifft_row.execute(mult, shared_row);

    // step 7: real part + scaling
    output[idx] = mult.x / static_cast<Precision>(WIDTH * HEIGHT);
}
