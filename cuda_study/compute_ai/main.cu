...
void generateRandomImageColumnMajor(int width, int height
...
void generateRandomKernelsColumnMajor(int width, int height
    , int n_terms
    ...
    )
{
    ...
    for (auto i = 0; i < n_terms; ++i) {
        generateRandomImageColumnMajor(width, height
            , kernels[i]
            , i
            );
